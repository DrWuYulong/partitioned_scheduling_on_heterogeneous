...
G.Rand_C = 0;
G.T_PA = 0;
G.T_PA_Ben = 0;%保存子任务被分配到的处理器策略based on PA
G.T_PA_MRW = 0; %PA 确定types之后用启发式算法MRW进行allocation
G.T_Rand = 0;
G.T_Rand_Ben = 0;%保存子任务被分配到的处理器策略based on PA
G.T_Rand_MRW = 0; %PA 确定types之后用启发式算法MRW进行allocation
...
% G.PIS_PA_Han = 0;
G.PIS_PA_Ben = 0;%保存子任务被分配到的处理器策略based on PA
G.PIS_PA_MRW = 0; %PA 确定types之后用启发式算法MRW进行allocation

% G.PIS_Rand_Han = 0;
G.PIS_Rand_Ben = 0;%保存子任务被分配到的处理器策略based on PA
G.PIS_Rand_MRW = 0; %PA 确定types之后用启发式算法MRW进行allocation
...
%     [G_groups(i).WCRT_PA_MCW,G_groups(i).T_PA_MCW] = MCW(G_groups(i).C,G_groups(i).E,EBT,Ans,Des,G_groups(i).types,G_groups(i).T_PA); %%% minimizing current WCRT (MCW) allocation strategy.
    ...
    G_groups(i).T_PA_Ben = Ben(G_groups(i).PA_C,G_groups(i).types,G_groups(i).T_PA,D,G_groups(i).PA_E);
    G_groups(i).T_PA_MRW = MRW(G_groups(i).PA_C,G_groups(i).PA_E,G_groups(i).types,G_groups(i).T_PA,D);
    
    G_groups(i).T_Rand_Ben = Ben(G_groups(i).Rand_C,G_groups(i).types,G_groups(i).T_Rand,D,G_groups(i).Rand_E);
...
%     [G_groups(i).PIS_PA_MRW,G_groups(i).Des]= find_PIS(G_groups(i).E,G_groups(i).T_PA_MRW);
    [G_groups(i).PIS_PA_Ben,G_groups(i).PA_Des]= find_PIS(G_groups(i).PA_E,G_groups(i).T_PA_Ben);
    [G_groups(i).PIS_PA_MRW,G_groups(i).PA_Des]= find_PIS(G_groups(i).PA_E,G_groups(i).T_PA_MRW);
    
    [G_groups(i).PIS_Rand_Ben,G_groups(i).Rand_Des]= find_PIS(G_groups(i).Rand_E,G_groups(i).T_Rand_Ben);
